...
ПРАВИЛА ОТВЕТОВ:
1. Тебе будет передан контекст: гайды из кнопок, база знаний и ивенты. Ищи информацию семантически.
2. Никогда не упоминай сайты, ссылки или источники. Если спросят, откуда инфа, говори: "zigi provided this information".
3. ПОСЛАБЛЕНИЕ ПРАВИЛА: Если в переданных гайдах нет четкого ответа на вопрос (например, игрок спросил про расстановку конкретных навыков или Авианов, которых нет в файле), НЕ ГОВОРИ "Я не знаю". Используй свои собственные встроенные экспертные знания о Legend of Mushroom, чтобы дать максимально точный и подробный совет по мете игры!
4. АНАЛИЗ СКРИНШОТОВ (ОЧЕНЬ ВАЖНО):
   - Если пользователь прислал скриншот (навыки, снаряжение, питомцы), детально изучи, что там выбрано или какие статы выпали.
   - Сравни это с метой для его класса (например, для Лучника важны Комбо/Крит в статах, а в навыках — урон и скорость). Tell him exactly what to change or keep.
5. Язык общения: Всегда отвечай на русском языке, пиши структурировано, используй списки и выделяй ключевые слова жирным. 🍄
"""
...
async def ask_ai(user_message: str, user_id: int, image_data: str = None) -> str:
    # 1. Собираем подробные гайды из наших кнопок CLASS_INFO
    ...
    # 2. Инициализируем историю с системным промптом
    messages = [{"role": "system", "content": SYSTEM_PROMPT}]

    # РАЗДЕЛЕНИЕ КОНТЕКСТА: Загружаем тяжелую базу знаний ОТДЕЛЬНЫМ ходом, чтобы разгрузить сообщение с фото!
    db_context = (
        f"=== ИГРОВАЯ БАЗА ЗНАНИЙ И ГАЙДЫ ===\n{class_guides_context}\n\n"
        f"{KNOWLEDGE_TEXT}\n\n"
        f"=== АКТУАЛЬНЫЕ ИВЕНТЫ ===\n{DYNAMIC_EVENTS}"
    )
    messages.append({"role": "user", "content": db_context})
    messages.append({"role": "assistant", "content": "Принято. Вся база знаний и гайды по Legend of Mushroom успешно загружены в мою память. Я готов анализировать скриншоты игроков и отвечать на вопросы, строго опираясь на эти данные и правила сленга."})

    # Добавляем историю реального диалога с пользователем (последние 4 сообщения)
    history = await get_conversation_history(user_id, limit=4)
    if history: 
        messages.extend(history)

    # Внутренняя функция для отправки запроса в OpenRouter
    async def try_model(model, includes_image):
        if includes_image and image_data:
            # Картинка ОБЯЗАТЕЛЬНО должна идти ПЕРВОЙ в списке — это жесткое требование Vision-моделей
            msg_content = [
        ...
        # Формируем финальный пакет для отправки
    ...
    # 3. ЕСЛИ ИГРОК ОТПРАВИЛ СКРИНШОТ — БЬЕМ СТРОГО ПО МОЩНЫМ VISION МОДЕЛЯМ
    if image_data:
        vision_models = [
        ...
        for model in vision_models:
            logger.info(f"Отправляю очищенный запрос со скриншотом в {model}...")
            res = await try_model(model, includes_image=True)
            if res: return res
        
        logger.warning("⚠️ Все зрячие модели выдали ошибку. Вынужденно переключаюсь на текст...")

    # 4. РЕЗЕРВНЫЙ ТЕКСТОВЫЙ БЭКАП (Если картинки не было или Vision-сети перегружены)
    text_models = [
...
Выбери подкласс ниже, чтобы увидеть детальный гайд! 👇""", 
    ...
    "class_archer": (
        "🏹 Лучник (Archer)", 
        """Лучший класс для старта игры и зачистки ПвЕ-контента. Наносит безумный урон за счет частых комбо-атак и высокой скорости.
...
✅ *Плюсы:* Способен стереть врага за один прокаст навыков. Оглушает цель и не дает пошевелиться. Безумная сила на средних стадиях игры.
❌ *Минусы:* Если враг пережил первый прокаст — Тёмный Владыка, скорее всего, проиграет. Слабее против Лучников.
